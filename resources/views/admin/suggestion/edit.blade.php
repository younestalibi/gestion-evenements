                        ...
                        <form action="{{ route('admin.suggestion.update', $suggestion->id) }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="col-md-4 col-sm-12 mb-3">
                                    ...
                                    <select class="form-control @error('name') is-invalid @enderror" id="name" name="name">
                                        @foreach (\App\Models\Service::POPULAR_CITIES as $key => $value)
                                            <option {{ old('name', $suggestion->name) == $key ? 'selected' : '' }} value="{{ $key }}">{{ $value }}</option>
                                        @endforeach
                                    </select>
                                    @error('name')
                            ...
                            <div class="row">
                                <div class="col-md-12 col-sm-12 mb-3">
                                    ...
                                    <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" placeholder="Description of suggestion" style="max-height:120px; height: 100px">{{ old('description', $suggestion->description) }}</textarea>
                                    @error('description')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="row">
                                <div class="col-md-12 col-sm-12 mb-3 d-flex justify-content-start align-items-center gap-4">
                                    <div class="col-5">
                                        <img src="{{ asset('users/suggestions/' . $suggestion->image) }}" alt="user-avatar" class="d-block rounded img-fluid" id="uploadedAvatar" />
                                    </div>
